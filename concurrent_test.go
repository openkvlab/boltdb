...
func mustOpenDB(t *testing.T, dbPath string, o *bolt.Options) *bolt.DB {
	t.Logf("Opening bolt DB at: %s", dbPath)
	if o == nil {
...
than previous read transaction. It verifies that bolt correctly
...
func TestConcurrentRepeatableRead(t *testing.T) {
	...
	for _, tc := range testCases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			// bolt will not release free pages directly after committing
