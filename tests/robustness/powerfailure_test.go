...
import (
	...
	"github.com/openkvlab/boltdb/tests/dmflakey"
)
...
func doPowerFailure(t *testing.T, du time.Duration, fsType dmflakey.FSType, mkfsOpt string, fsMountOpt string, useFailpoint bool) {
	...
	case <-time.After(10 * time.Second):
		t.Log("boltdb is supposed to be already stopped, but actually not yet; forcibly kill it")
		assert.NoError(t, cmd.Process.Kill())
	case err := <-errCh:
		require.Error(t, err)
	}
	...
	t.Logf("verify data")
	output, err := exec.Command("boltdb", "check", dbPath).CombinedOutput()
	require.NoError(t, err, "boltdb check output: %s", string(output))
}
